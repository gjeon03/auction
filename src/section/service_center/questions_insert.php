...
if(strcmp($files["name"][0], "")){//보낸 파일이 없다면 for문을 불러들이지 않는다. 이걸 안하면 무조건 한번은 for문이 돌아서 에러 메시지가 뜸.
    ...
    $upload_dir = '/usr/local/apache2.4/htdocs/ac_questions/'; //물리적 저장위치
    for ($i=0; $i<$count; $i++){
...
if (!$title||!$content){
    ...
    $sql = "INSERT INTO question (id, name, nick, subject, content, regist_day, file_name_0, file_name_1, file_name_2, file_name_3, file_name_4, file_copied_0, file_copied_1, file_copied_2, file_copied_3, file_copied_4, hit, sns_type) VALUES ('$id', '$name', '$nick', '$title', '$content', now(), '$upfile_name[0]', '$upfile_name[1]', '$upfile_name[2]', '$upfile_name[3]', '$upfile_name[4]', '$copied_file_name[0]', '$copied_file_name[1]', '$copied_file_name[2]', '$copied_file_name[3]', '$copied_file_name[4]', 0, '$sns_type')";
